...
Scope note:
This script computes GT-conditioned prompt-comparison statistics.
Unmatched GT misses remain in the dataset with zero mask quality, while
false-positive predictions are excluded from these aggregates and reported
separately in failure-taxonomy outputs generated by the main pipeline.
...
def load_raw(raw_dir: pathlib.Path) -> pd.DataFrame:
    """Load per-instance records for GT-conditioned statistical analysis."""
    records = []
    skipped = 0
    ...
    # Exclude false-positive predictions to keep GT-conditioned aggregates.
    # Failure-taxonomy tables include false positives explicitly.
